...
public:
	...
	*/
	...
	Invertir Cola
	*/
	void invertirElementos(int k) {
		if (k >= 2) {
			int i = 1;
			Nodo *auxPosK = _prim;

			while (i < k) {
				auxPosK = auxPosK->_sig;
				i++;
			}

			Nodo *auxPosKSig = auxPosK->_sig;
			auxPosK->_sig = auxPosK->_ant;
			auxPosK->_ant = NULL;
			_prim = auxPosK;
			
			while (k > 1) {
				auxPosK = auxPosK->_sig;
				Nodo *aux = auxPosK->_ant;
				auxPosK->_ant = auxPosK->_sig;
				auxPosK->_sig = aux;
				k--;
			}
			if (auxPosKSig != NULL) {
				auxPosK->_sig = auxPosKSig;
			}
			else {
				_ult = auxPosK;
			}
