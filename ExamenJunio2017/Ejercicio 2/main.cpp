...
/**
  COMPLEJIDAD: Explica aquí justificadamente cuál es la complejidad de 
	...
	0(n log n) ya que se recorre el arbol entero.
*/

unsigned int singularesAux(Arbin<int> a, int &suma, int sumaAnt) {
	if (a.esVacio()) {
		return 0;
	}
	else {
		unsigned int singulares = 0, singularesIz = 0, singularesDer = 0;
		int sumaIz = 0, sumaDer = 0;

		singularesIz = singularesAux(a.hijoIz(), sumaIz, sumaAnt + a.raiz());
		singularesDer = singularesAux(a.hijoDer(), sumaDer, sumaAnt + a.raiz());

		if (sumaAnt == sumaIz + sumaDer) {
			singulares++;
		}

		suma = a.raiz() + sumaIz + sumaDer;

		return singulares + singularesIz + singularesDer;
	}
}

unsigned int numero_singulares(Arbin<int> a) {
	// A IMPLEMENTAR
	int suma = 0, sumaAnt = 0;
	return singularesAux(a, suma, sumaAnt);
}
