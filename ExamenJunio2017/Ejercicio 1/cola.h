...
public:
	...
	La complejidad es pos + dist porque primero se desplaza hasta la posicion del elemento y despues se mueve
	segun la distancia. O(pos+dist)
...
void desplaza(unsigned int pos, unsigned int dist) {
	...
	Nodo *auxAnt = _prim; //Elemento anterior al desplazado
	Nodo *aux = _prim->_sig; //Elemento que vamos a desplazar
	
	if (pos - 1 > 0) {
		for (int i = 1; i < pos - 1; i++) { //Comprobar despues si pos = 0;
			auxAnt = auxAnt->_sig;
			aux = aux->_sig;
		}

		if (dist != 0) {
			auxAnt->_sig = aux->_sig; //Nodo anterior al desplazado apunta ahora al nodo siguiente del desplazado;
			if (auxAnt->_sig == NULL) {
				_ult = auxAnt;
			}

			Nodo *despAnt = _prim;
			Nodo *desp = _prim->_sig;

			if ((pos > dist) && (pos - dist - 1 > 0)) {
				for (int i = 1; i < pos - dist - 1; i++) {
					despAnt = despAnt->_sig;
					desp = desp->_sig;
				}
